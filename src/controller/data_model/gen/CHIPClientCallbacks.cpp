...
#define GET_ATTRIBUTE_RESPONSE_CALLBACKS(name, obj)                                                                                \
    Callback::Cancelable * onSuccessCallback = nullptr;                                                                            \
    Callback::Cancelable * onFailureCallback = nullptr;                                                                            \
    NodeId sourceNodeId                      = obj->GetSourceNodeId();                                                             \
    uint8_t seq                              = static_cast<uint8_t>(obj->GetAppIdentifier());                                      \
    /* #6559: Currently, we only have one commands for the IMInvokeCommands and to a device, so the seqNum is always set to 0. */  \
...
bool IMSubscribeResponseCallback(const chip::app::SubscribeClient * apSubscribeClient, EmberAfStatus status)
{
    ...
    LogStatus(status);
    GET_ATTRIBUTE_RESPONSE_CALLBACKS("emberAfSubscribeResponseCallback", apSubscribeClient);
    if (status == EMBER_ZCL_STATUS_SUCCESS)
    {
        Callback::Callback<DefaultSuccessCallback> * cb =
            Callback::Callback<DefaultSuccessCallback>::FromCancelable(onSuccessCallback);
                                            ...
                                            TLV::TLVReader * apData, Protocols::InteractionModel::ProtocolCode status)
{
    ...
    {
        Callback::Cancelable * onSuccessCallback = nullptr;
        app::TLVSubscribeDataFilter tlvFilter    = nullptr;
        CHIP_ERROR err = gCallbacks.GetReportCallback(sourceId, aPath.mEndpointId, aPath.mClusterId, aPath.mFieldId,
                                                      &onSuccessCallback, &tlvFilter);
        if (CHIP_NO_ERROR == err)
        {
            tlvFilter(apData, onSuccessCallback);
        }
    }
    else
    {
        ...
        CHIP_ERROR err =
            gCallbacks.GetResponseCallback(sourceId, sequenceNumber, &onSuccessCallback, &onFailureCallback, &tlvFilter);
