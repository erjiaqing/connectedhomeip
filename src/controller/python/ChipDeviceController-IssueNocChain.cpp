...
exit:
    if (err == CHIP_NO_ERROR)
    {
        pychip_DeviceController_IssueNOCChainCallbackPythonCallbackFunct(
            context, err, chipNocSpan.data(), chipNocSpan.size(), chipIcacSpan.data(), chipIcacSpan.size(), chipRcacSpan.data(),
            chipRcacSpan.size(), ipkData.data(), ipk.HasValue() ? ipkData.size() : 0, adminSubject.ValueOr(kUndefinedNodeId));
    }
    else
    {
        pychip_DeviceController_IssueNOCChainCallbackPythonCallbackFunct(context, err, nullptr, 0, nullptr, 0, nullptr, 0, nullptr,
                                                                         0, 0);
    }
}
