...
ChipError::StorageType pychip_DeviceController_OnNetworkCommission(chip::Controller::DeviceCommissioner * devCtrl, uint64_t aNodeId,
                                                                   uint32_t aSetupPinCode, const uint8_t aFilterType,
                                                                   const char * aFilterParam);
...
ChipError::StorageType pychip_DeviceController_OnNetworkCommission(chip::Controller::DeviceCommissioner * devCtrl, uint64_t aNodeId,
                                                                   ...
                                                                   const char * aFilterParam)
{
    Dnssd::DiscoveryFilter filter(static_cast<Dnssd::DiscoveryFilterType>(aFilterType));
    switch (static_cast<Dnssd::DiscoveryFilterType>(aFilterType))
    {
    ...
    case chip::Dnssd::DiscoveryFilterType::kDeviceType:
        sscanf(aFilterParam, "%" SCNu64, &(filter.code));
        break;
    ...
    case chip::Dnssd::DiscoveryFilterType::kInstanceName:
        filter.code         = 0;
        filter.instanceName = aFilterParam;
        break;
    default:
        return CHIP_ERROR_INVALID_ARGUMENT.AsInteger();
    }

    sPairingDeviceDiscoveryDelegate.Setup(aNodeId, aSetupPinCode, sCommissioningParameters, &sPairingDelegate, devCtrl);
