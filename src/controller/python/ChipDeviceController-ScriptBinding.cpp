...
PyChipError pychip_DeviceController_StackInit(Controller::Python::StorageAdapter * storageAdapter, bool enableServerInteractions)
{
    ...
    ReturnErrorOnFailure(sPersistentStorageOpCertStore.Init(storageAdapter));
...
PyChipError pychip_DeviceController_StackShutdown()
{
    ...
    //
    ReturnErrorOnFailure(chip::DeviceLayer::PlatformMgr().StopEventLoopTask());
                                                      ...
                                                      char * outAddress, uint64_t maxAddressLen, uint16_t * outPort)
{
    ...
    ReturnErrorOnFailure(devCtrl->GetPeerAddressAndPort(nodeId, address, *outPort));
    VerifyOrReturnError(address.ToString(outAddress, static_cast<uint32_t>(maxAddressLen)), CHIP_ERROR_BUFFER_TOO_SMALL);
                                              ...
                                              uint32_t setupPINCode, chip::NodeId nodeid)
{
    ...
    return devCtrl->PairDevice(nodeid, params, sCommissioningParameters);
}

PyChipError pychip_DeviceController_ConnectWithCode(chip::Controller::DeviceCommissioner * devCtrl, const char * onboardingPayload,
                                                    chip::NodeId nodeid)
{
    return devCtrl->PairDevice(nodeid, onboardingPayload, sCommissioningParameters);
}
...
PyChipError pychip_DeviceController_SetThreadOperationalDataset(const char * threadOperationalDataset, uint32_t size)
{
    ReturnErrorCodeIf(!sThreadBuf.Alloc(size), CHIP_ERROR_NO_MEMORY);
...
PyChipError pychip_DeviceController_SetWiFiCredentials(const char * ssid, const char * credentials)
{
    ...
    ReturnErrorCodeIf(!sSsidBuf.Alloc(ssidSize), CHIP_ERROR_NO_MEMORY);
    memcpy(sSsidBuf.Get(), ssid, ssidSize);

    size_t credsSize = strlen(credentials);
    ReturnErrorCodeIf(!sCredsBuf.Alloc(credsSize), CHIP_ERROR_NO_MEMORY);
                                                           ...
                                                           uint32_t setupPINCode, chip::NodeId nodeid)
{
    ...
    addr.SetTransportType(chip::Transport::Type::kUdp).SetIPAddress(peerAddr);
    params.SetPeerAddress(addr).SetDiscriminator(0);
    return devCtrl->EstablishPASEConnection(nodeid, params);
}
PyChipError pychip_DeviceController_Commission(chip::Controller::DeviceCommissioner * devCtrl, chip::NodeId nodeid)
{
...
struct GetDeviceCallbacks
{
    ...
    static void OnDeviceConnectedFn(void * context, Messaging::ExchangeManager & exchangeMgr, SessionHandle & sessionHandle)
    {
        ...
        self->mCallback(operationalDeviceProxy, CHIP_NO_ERROR);
        delete self;
    }
...
PyChipError pychip_GetLocalSessionId(chip::OperationalDeviceProxy * deviceProxy, uint16_t * localSessionId)
{
    VerifyOrReturnError(deviceProxy->GetSecureSession().HasValue(), CHIP_ERROR_MISSING_SECURE_SESSION);
    VerifyOrReturnError(localSessionId != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
...
{
    VerifyOrReturnError(deviceProxy->GetSecureSession().HasValue(), CHIP_ERROR_MISSING_SECURE_SESSION);
    VerifyOrReturnError(numSessions != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
...
PyChipError pychip_GetDeviceBeingCommissioned(chip::Controller::DeviceCommissioner * devCtrl, chip::NodeId nodeId,
                                              CommissioneeDeviceProxy ** proxy)
{
    return devCtrl->GetDeviceBeingCommissioned(nodeId, proxy);
}

// This is a method called VERY seldom, just for RemoveFabric/UpdateNOC
PyChipError pychip_ExpireSessions(chip::Controller::DeviceCommissioner * devCtrl, chip::NodeId nodeId)
{
    VerifyOrReturnError((devCtrl != nullptr) && (devCtrl->SessionMgr() != nullptr), CHIP_ERROR_INVALID_ARGUMENT);
