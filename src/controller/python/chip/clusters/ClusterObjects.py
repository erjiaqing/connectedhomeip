...
class ClusterObjectDescriptor:
    ...
    def TLVToDict(self, tlvBuf: bytes) -> Dict[str, Any]:
        print(tlv.TLVReader(tlvBuf).get())
...
class ClusterCommand(ClusterObject):
    ...
    @classmethod
    @property
    def ResponseType(self) -> typing.Union['ClusterCommand', None]:
        return None

    async def Send(self, nodeId: int, endpointId: int):
        from . import Command as cmd
        return await cmd.GetDeviceController().SendCommand(nodeId=nodeId, endpointId=endpointId, payload=self)
