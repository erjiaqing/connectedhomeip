...
using OnCommandSenderErrorCallback    = void (*)(PyObject appContext,
                                              ...
                                              uint32_t chiperror);
using OnCommandSenderFinalCallback    = void (*)(PyObject appContext);
...
OnCommandSenderFinalCallback gOnCommandSenderFinalCallback       = nullptr;
...
public:
    ...
    {
        gOnCommandSenderFinalCallback(mAppContext);
...
void pychip_CommandSender_InitCallbacks(OnCommandSenderResponseCallback onCommandSenderResponseCallback,
                                        ...
                                        OnCommandSenderFinalCallback onCommandSenderFinalCallback)
{
    ...
    gOnCommandSenderFinalCallback    = onCommandSenderFinalCallback;
}
                                      ...
                                      chip::ClusterId clusterId, chip::CommandId commandId, const uint8_t * payload, size_t length)
{
...

    {
        ...
        while (reader.Next() == CHIP_NO_ERROR)
        {
            ...
            tReader.Init(reader);
            ChipLogError(Controller, "Copying element: tag %" PRIx64 " type %d", tReader.GetTag(), int(tReader.GetType()));
