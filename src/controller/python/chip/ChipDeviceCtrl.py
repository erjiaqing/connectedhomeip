...
_GenerateNOCChainCallbackPythonCallbackFunct = CFUNCTYPE(
    None, py_object, c_uint32, c_void_p, c_size_t, c_void_p, c_size_t, c_void_p, c_size_t, c_void_p, c_size_t, c_uint64)


@_GenerateNOCChainCallbackPythonCallbackFunct
def _GenerateNOCChainCallbackPythonCallback(devCtrl, status: int, noc: c_void_p, nocLen: int, icac: c_void_p, icacLen: int, rcac: c_void_p, rcacLen: int, ipk: c_void_p, ipkLen: int, adminSubject: int):
...
class ChipDeviceController():
    ...
    def __init__(self, opCredsContext: ctypes.c_void_p, fabricId: int, fabricIndex: int, nodeId: int, paaTrustStorePath: str = "", useTestCommissioner: bool = False):
        ...
        self._dmLib.pychip_DeviceController_SetGenerateNOCChainCallbackPythonCallback(_GenerateNOCChainCallbackPythonCallback)
    ...
    def NOCChainCallback(self, status, noc, icac, rcac, ipk, adminSubject):
        ...
        print(f"RCAC:         {rcac}")
        print(f"IPK:          {ipk}")
        print(f"ADMINSubject: {adminSubject}")
    ...
    def GenerateNOCChain(self, csr: Clusters.OperationalCredentials.Commands.CSRResponse, nodeId: int):
        self.CheckIsActive()

        return self._ChipStack.CallAsync(
            lambda: self._dmLib.pychip_DeviceController_GenerateNOCChain(
    ...
    def _InitLib(self):
        if self._dmLib is None:
            ...
            self._dmLib.pychip_DeviceController_GenerateNOCChain.argtypes = [
                c_void_p, py_object, c_char_p, c_size_t, c_uint64
            ]
            self._dmLib.pychip_DeviceController_GenerateNOCChain.restype = c_uint32

            self._dmLib.pychip_DeviceController_SetGenerateNOCChainCallbackPythonCallback.argtypes = [
                _GenerateNOCChainCallbackPythonCallbackFunct]
            self._dmLib.pychip_DeviceController_SetGenerateNOCChainCallbackPythonCallback.restype = None
