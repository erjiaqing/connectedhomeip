...
struct ClusterInfo
{
    ...
    {
        VerifyOrReturnError(!HasValidEndpointId() || mEndpointId == other.mEndpointId, false);
        VerifyOrReturnError(!HasValidClusterId() || mClusterId == other.mClusterId, false);
        VerifyOrReturnError(!HasValidAttributeId() || mFieldId == other.mFieldId, false);
        VerifyOrReturnError(!HasValidListIndex() || mListIndex == other.mListIndex, false);

        return true;
    }

    bool HasWildcard() const { return !HasValidEndpointId() || !HasValidClusterId() || !HasValidAttributeId(); }
    ...
    inline bool HasValidNodeId() const { return mNodeId != kUndefinedNodeId; }
    inline bool HasValidEndpointId() const { return mEndpointId != kInvalidEndpointId; }
    inline bool HasValidClusterId() const { return mClusterId != kInvalidClusterId; }
    inline bool HasValidAttributeId() const { return mFieldId != kInvalidAttributeId; }
