...
void TestAllWildcard(nlTestSuite * apSuite, void * apContext)
{
    ...
    for (app::PathIterator iter(&clusInfo); iter.Get(path); iter.Proceed())
    {
...
{
    app::ClusterInfo clusInfo;
    clusInfo.mClusterId.SetValue(Test::MockClusterId(3));
    clusInfo.mFieldId.SetValue(Test::MockAttributeId(3));
...
{
    app::ClusterInfo clusInfo;
    clusInfo.mEndpointId.SetValue(Test::kMockEndpoint3);
    clusInfo.mFieldId.SetValue(app::Clusters::Globals::Attributes::ClusterRevision::Id);
...
{
    app::ClusterInfo clusInfo;
    clusInfo.mEndpointId.SetValue(Test::kMockEndpoint2);
    clusInfo.mClusterId.SetValue(Test::MockClusterId(3));
...
void TestMultipleClusInfo(nlTestSuite * apSuite, void * apContext)
{
    ...
    clusInfo2.mClusterId.SetValue(Test::MockClusterId(3));
    clusInfo2.mFieldId.SetValue(Test::MockAttributeId(3));

    app::ClusterInfo clusInfo3;
    clusInfo3.mEndpointId.SetValue(Test::kMockEndpoint3);
    clusInfo3.mFieldId.SetValue(app::Clusters::Globals::Attributes::ClusterRevision::Id);

    app::ClusterInfo clusInfo4;
    clusInfo4.mEndpointId.SetValue(Test::kMockEndpoint2);
    clusInfo4.mClusterId.SetValue(Test::MockClusterId(3));

    app::ClusterInfo clusInfo5;
    clusInfo5.mEndpointId.SetValue(Test::kMockEndpoint2);
    clusInfo5.mClusterId.SetValue(Test::MockClusterId(3));
    clusInfo5.mFieldId.SetValue(Test::MockAttributeId(3));
    ...
    for (app::PathIterator iter(&clusInfo1); iter.Get(path); iter.Proceed())
    {
        ChipLogDetail(AppServer, "Visited Attribute: 0x%04" PRIX16 " / " ChipLogFormatMEI " / " ChipLogFormatMEI, path.mEndpointId,
                      ChipLogValueMEI(path.mClusterId), ChipLogValueMEI(path.mAttributeId));
        // NL_TEST_ASSERT(apSuite, index < ArraySize(paths) && paths[index] == path);
...
int TestPathIterator()
{
    nlTestRunner(&sSuite, nullptr);
    return (nlTestRunnerStats(&sSuite));
}

CHIP_REGISTER_TEST_SUITE(TestPathIterator)
