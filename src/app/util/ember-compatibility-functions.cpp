...
CHIP_ERROR ReadSingleClusterData(ClusterInfo & aClusterInfo, TLV::TLVWriter * apWriter, bool * apDataExists)
{
    ...
    EmberAfStatus status = emberAfReadAttribute(aClusterInfo.mEndpointId, aClusterInfo.mClusterId, aClusterInfo.mFieldId,
                                                CLUSTER_MASK_SERVER, data, sizeof(data), &dataType);
    ...
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        ...
        err =
            apWriter->Put(chip::TLV::ContextTag(AttributeDataElement::kCsTag_Status), (uint16_t) 0x86 /* unsupported attribute */);
        ExitNow();
    }
    ...
    case ZCL_OCTET_STRING_ATTRIBUTE_TYPE: // Octet string
        SuccessOrExit(err = apWriter->PutBytes(TLV::ContextTag(AttributeDataElement::kCsTag_Data), data + 1, data[0]));
        break;
    case ZCL_ARRAY_ATTRIBUTE_TYPE: {
        // TODO: Add support to arrays & structures
